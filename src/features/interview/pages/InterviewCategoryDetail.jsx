    ...
    <div className="h-screen bg-[#030014] text-[#e2e8f0] font-sans flex flex-col overflow-hidden relative">
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <aside className="w-80 md:w-96 border-r border-[#6366f1]/12 bg-[#050518]/25 backdrop-blur-sm flex flex-col shrink-0 select-none">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-width-none text-left" style={{ scrollbarWidth: "none" }}>
            {filteredQuestions.length === 0 ? (
              ...
              (() => {
                ...
                // treeNode = { name, questions: [], children: { subName: treeNode } }
                const tree = { name: "Root", questions: [], children: {} };

                filteredQuestions.forEach((q) => {
                  const sub = q.subcategory || "General";
                  // Split by slashes (trimmed)
                  const parts = sub.split("/").map(p => p.trim()).filter(Boolean);
                  
                  let currentNode = tree;
                  parts.forEach((part) => {
                    if (!currentNode.children[part]) {
                      currentNode.children[part] = { name: part, questions: [], children: {} };
                    }
                    currentNode = currentNode.children[part];
                  });
                  currentNode.questions.push(q);
                });

                // Recursive component to render tree nodes with proper nesting indentations
                const renderTreeNode = (node, depth = 0, path = "Root") => {
                  ...
                  return (
                    <div key={node.name} className="space-y-1 w-full">
                      {/* Folder Name (omit root label) */}
                      {node.name !== "Root" && (
                        <button 
                          onClick={() => toggleFolder(currentPath)}
                          className="w-full text-[10px] font-bold font-mono uppercase tracking-widest text-[#a855f7]/70 hover:text-[#a855f7] transition-colors flex items-center gap-1.5 py-1 pl-1 cursor-pointer select-none text-left"
                      ...
                      {/* Content block with collapse height animation support */}
                      {!isCollapsed && (
                        <div className="space-y-1 w-full">
                          ...
                          {/* Render Questions under this folder */}
                          {hasQuestions && (
                            <div 
